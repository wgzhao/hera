...
public class BeanConvertUtils {
    ...
    public static HeraDebugHistoryVo convert(HeraDebugHistory heraDebugHistory) {
        ...
        if (heraDebugHistory.getStartTime() != null) {
            heraJobHistoryVo.setStartTime(sdf.format(heraDebugHistory.getStartTime()));
        }
        if (heraDebugHistory.getEndTime() != null) {
            heraJobHistoryVo.setEndTime(sdf.format(heraDebugHistory.getEndTime()));
        }
        if (heraDebugHistory.getGmtCreate() != null) {
            heraJobHistoryVo.setGmtCreate(sdf.format(heraDebugHistory.getGmtCreate()));
        }
        if (heraDebugHistory.getGmtModified() != null) {
            heraJobHistoryVo.setGmtModified(sdf.format(heraDebugHistory.getGmtModified()));
        }
    ...
    public static HeraDebugHistory convert(HeraDebugHistoryVo jobHistoryVo) {
        ...
        try {
            if (jobHistoryVo.getStartTime() != null) {
                jobHistory.setStartTime(sdf.parse(jobHistoryVo.getStartTime()));
            }
            if (jobHistoryVo.getEndTime() != null) {
                jobHistory.setEndTime(sdf.parse(jobHistoryVo.getEndTime()));
            }
            if (jobHistoryVo.getGmtCreate() != null) {
                jobHistory.setGmtCreate(sdf.parse(jobHistoryVo.getGmtCreate()));
            }
            if (jobHistoryVo.getGmtModified() != null) {
                jobHistory.setGmtModified(sdf.parse(jobHistoryVo.getGmtModified()));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public static HeraActionVo transform(HeraAction action) {
        ...
        heraActionVo.setAuto(action.getAuto().equals(auto));
        return heraActionVo;
    }
    ...
    public static void stringToMap(String str, Map<String, String> configs) {
        ...
        Arrays.stream(split).forEach(x -> {
            int index = x.indexOf("=");
            if (index != -1) {
                configs.put(x.substring(0, index).trim(), x.substring(index + 1).trim());
            }
        });
    }
