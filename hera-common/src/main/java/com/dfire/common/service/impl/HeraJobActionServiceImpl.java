...
public class HeraJobActionServiceImpl
        implements HeraJobActionService
{
    ...
    private boolean isNeedUpdateAction(HeraAction heraAction, Long nowAction)
    {
        ...
        if (action != null) {
            ...
            if (!StatusEnum.RUNNING.toString().equals(action.getStatus())) {
                ...
                heraAction.setConfigs(action.getConfigs()); //处理重刷历史数据bug
            }
            else {
    ...
    @Override
    public String findJobConfig(String actionId)
    {
        return heraJobActionMapper.findJobConfig(actionId);
    }
    ...
    //获取最近版本的id和状态
    @Override
    public String getLatestVersionAndStatus(String jobId)
    {
        return heraJobActionMapper.getLatestVersionAndStatus(jobId);
    }
    //获取最近版本的id和状态
    ...
    {
        return heraJobActionMapper.getStatus(jobId);
    }

    //获取最近版本的id
    @Override
    public String getLatestVersionId(String jobId)
    {
        return heraJobActionMapper.getLatestVersionId(jobId);
    }

    //获取上游任务的状态
    public List<String> judgeUpDependsStatus(String id)
    {
        // public String judgeUpDependsStatus(String id){
        return heraJobActionMapper.judgeUpDependsStatus(id);
    }

    //获取上游任务的id
    public String getUpDependsId(String id)
    {
        // public String judgeUpDependsStatus(String id){
        return heraJobActionMapper.getUpDependsId(id);
    }

    //获取下游任务的Id和状态
    public List<String> judgeDownDependsStatus(String id)
    {
        // public String judgeUpDependsStatus(String id){
        return heraJobActionMapper.judgeDownDependsStatus(id);
    }

    //获取下游任务的id
    public List<String> judgeDownDependsId(String id)
    {
        // public String judgeUpDependsStatus(String id){
        return heraJobActionMapper.judgeDownDependsId(id);
    }

    //获取任务的依赖和已准备好的依赖
    public String getReadDependency(String id)
    {
        return heraJobActionMapper.getReadDependency(id);
    }

    //获取任务运行的常规时间
    public String getTaskCommonTime(String id)
    {
        return heraJobActionMapper.getTaskCommonTime(id);
    }
}
