layui.use("layer", function () {
    ...
    function initPieJobStatus(status) {
        ...
        myChart.on('click', function (param) {
            if (param.data.name == "failed" || param.data.name == "success" || param.data.name == "running") {
                location.href = base_url + "/stopJobDetail?syFlag=0";
            }
        })
    }
    ...
    function initJobTopTen(jobs) {
        ...
        jobs.forEach(function (job, index) {
            ...
            //xAxis[index] = job.jobId+"("+job.jobName+")";
            xAxis[index] = {
                value: job.jobId + "(" + job.jobName + ")", textStyle: {
                    fontSize: 8
                }
            }
        })
...
function getStopHeraJobInfo() {
    let url = base_url + "/scheduleCenter/getStopHeraJobInfo";
...
function getSqoopFailedNum() {
    let url = base_url + "/sqoopTaskCenter/getSqoopFailedNum";
    $.get(url, function (data) {
        $('#SqoopFailedCount').text(data);
    });
}
...
function getFindNum() {
    /**
     ...
     　　*/
    let url = base_url + "/homePage/getDataFindNumNew";
    let parameter;
    $.get(url, parameter, function (data) {
        $('#dataFindNum').text(data);
    });
}

getFindNum()

function getMaxwellCount() {
    /**
     　　* @Description: TODO 首页maxwell监控结果
     　　* @param
     　　* @return
     　　* @throws
     　　* @author lenovo
     ...
     　　*/
    let url = base_url + "/homePage/getMaxwellCount";
    let parameter;
    $.get(url, parameter, function (data) {
        $('#maxwellCount').text(data);
    });
}

//getMaxwellCount()

function getNginxCount() {
    /**
     　　* @Description: TODO 首页nginx监控结果
     　　* @param
     　　* @return
     　　* @throws
     　　* @author lenovo
     　　* @date 2019/11/7 20:27
     　　*/
    let url = base_url + "/homePage/getNginxActiveCount";
    let parameter;
    $.get(url, parameter, function (data) {
        $('#nginxCount').text(data);
    });
}
...
function getCurrentUser2() {
    ...
    $.get(url, parameter, function (data) {
        //    layer.msg(data);
        userName = "欢迎，" + data + "，祝你开心每一天！";
        $("#userNameInfo").text(userName)

        let url = base_url + "/homePage/getUserJobInfo";
        let parameter = {
            "owner": data
        };
        $.get(url, parameter, function (data) {
            $("#tdJobCount").text(data.data.jobCount)
            $("#tdManJobCount").text(data.data.manJobCount)
            $("#tdFailedJobCount").text(data.data.failedJobCount)
            $("#allJobsNum").text(data.data.jobStartCount)
            $("#tdFailedJobCount").css("color", "red")
        });
    });
}
