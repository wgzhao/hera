...
public class WorkHandler extends SimpleChannelInboundHandler<SocketMessage> {
    ...
    private final WorkHandlerRequest handlerRequest = new WorkHandlerRequest();

    private final CompletionService<ChannelResponse> completionService = new ExecutorCompletionService<>(
    ...
    private final ConcurrentHashMap<Channel, HeraChannel> channelMap = new ConcurrentHashMap<>(2);
    ...
    public WorkHandler(final WorkContext workContext) {
        ...
        Executor executor = Executors.newSingleThreadExecutor(
                new NamedThreadFactory("send-message-to-master-thread", true));
            ...
            case REQUEST:
                ...
                switch (request.getOperate()) {
                    ...
                    case Debug:
                        ...
                        break;
                    case ReRun:
                        System.out.println("channelRead0--ReRun---" + socketMessage.getBody());
                        completionService.submit(() ->
                                new ChannelResponse(getChannel(channel), new WorkExecuteJob().execute(workContext, request).get()));
                        break;
                    case Cancel:
