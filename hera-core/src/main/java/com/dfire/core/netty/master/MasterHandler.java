...
public class MasterHandler extends ChannelInboundHandlerAdapter {
    ...
    private final CompletionService<ChannelResponse> completionService;
    ...
    public MasterHandler(MasterContext masterContext) {
        ...
        completionService = new ExecutorCompletionService<>(
                new ThreadPoolExecutor(
                        ...
                        new NamedThreadFactory("master-execute", false),
                        new ThreadPoolExecutor.AbortPolicy()));

        ThreadPoolExecutor executor = new ThreadPoolExecutor(
                1, 1, 60L, TimeUnit.SECONDS,
                new LinkedBlockingQueue<>(),
                new NamedThreadFactory("master-deal", false),
                new ThreadPoolExecutor.AbortPolicy());
        ...
        executor.execute(() -> {
                    ...
                    while (true) {
                        try {
                            ...
                            ErrorLog.error(LogHelpUtil.getSpecificTrace(throwable));
...
//                    case CancelForTime:
//                        completionService.submit(() ->
//                                new ChannelResponse(FailBackCluster.wrap(channel), MasterHandlerWebResponse.handleWebCancelForTime(masterContext, webRequest)));
//                        break;
                    case UpdateJob:
                        completionService.submit(() ->
