...
public class WorkClient {
    ...
    public void init() {
        ...
        workSchedule.schedule(new Runnable() {
            ...
            private final WorkerHandlerHeartBeat workerHandlerHeartBeat = new WorkerHandlerHeartBeat();
            ...
            public void run() {
                try {
                    ...
                    workSchedule.schedule(this, (long) (failCount + 1) * HeraGlobalEnvironment.getHeartBeat(), TimeUnit.SECONDS);
                }
            }
        ...
        workSchedule.scheduleWithFixedDelay(new Runnable() {
            ...
            private void printScheduleLog(Job job, Exception e) {
                try {
                    ...
                    ErrorLog.error(LogHelpUtil.getSpecificTrace(ex));
    ...
    public String cancelJobFromWebForTime(ExecuteKind kind, String id) throws ExecutionException, InterruptedException {
        RpcWebResponse.WebResponse webResponse = WorkerHandleWebRequest.handleCancelForTime(workContext, kind, id).get();
        if (webResponse.getStatus() == ResponseStatus.Status.ERROR) {
            ErrorLog.error(webResponse.getErrorText());
            return webResponse.getErrorText();
        }
